/* eslint-disable react/jsx-no-undef */
import Image from "next/image";
import Link from "next/link";

import { Card, CardHeader } from "@nextui-org/react";
import {
  IconBattery3,
  IconBrandSpeedtest,
  IconCarFilled,
  IconChartCovariate,
  IconEngine,
  IconGasStation,
  IconHorse,
  IconManualGearbox,
  IconSettings,
} from "@tabler/icons-react";

import getMarkaDetails from "@/app/actions/getmarkadets";
import getVersiyon from "@/app/actions/getversiyon";

type Params = Promise<{ kasaid: string }>;

export default async function Model({ params }: { params: Params }) {
  const kasaid = (await params).kasaid;
  const verss = await getVersiyon({ kasaid: kasaid });
  const marka = await getMarkaDetails({ kasaid: kasaid });

  return (
    <>
      <div className="relative">
        {/* Hero Section */}
        <div
          className="mt-16 h-[350px] w-full animate-fade-down items-center justify-center bg-cover bg-center object-contain animate-delay-500 animate-normal animate-fill-both animate-once animate-ease-in"
          style={{ backgroundImage: "url('/mainpage.png')" }}
        >
          <div className="grid grid-cols-2">
            <div className="col-span-1 mx-auto animate-fade-right px-5 animate-delay-1000 animate-normal animate-fill-both animate-once animate-ease-in">
              <br />
              <br />
              <p className="text-4xl tracking-widest text-black lg:text-5xl">
                {marka?.marka?.name}
              </p>
              <br />
              <p className="text-4xl tracking-widest text-black lg:text-5xl">
                {marka?.modelAd}
              </p>
            </div>
            <div className="col-span-1 mx-auto h-[100px] w-[100px] animate-fade-left animate-delay-1000 animate-normal animate-fill-both animate-once animate-ease-in lg:h-[200px] lg:w-[200px]">
              <br />
              <br />
              <Image
                height={1000}
                width={1000}
                alt="Card background"
                className="z-0 h-full w-full object-contain"
                src={marka.marka?.image || "/logooo.png"}
                quality={100}
              />
            </div>
          </div>
        </div>
        <br />
        <br />
        <br />
        <div className="relative z-0">
          <div className="mx-auto mt-10 grid grid-cols-12 gap-10 lg:px-32">
            <p className="col-span-12 mt-5 h-full justify-end text-center text-2xl tracking-wider text-black md:col-span-6 md:text-start md:text-4xl lg:mt-20 lg:text-5xl">
              İstediğiniz versiyonu seçiniz <br />
              <br />
            </p>
            <div className="col-span-12 mx-auto h-full px-10 md:col-span-6">
              <div key="123">
                <p className="mb-3 text-center text-2xl tracking-widest text-black/80 md:text-3xl lg:text-4xl">
                  {marka.kasalar?.name}
                </p>
                <Card shadow="lg">
                  <Image
                    height={2000}
                    width={2000}
                    alt="Card background"
                    className="z-0 h-full w-full object-contain"
                    src={marka.kasalar?.image || "/img/bmw.png"}
                    quality={100}
                  />
                </Card>
              </div>
            </div>

            {verss.map((item, index) => (
              <div
                className="col-span-12 px-10 md:col-span-6 md:px-3 lg:col-span-4"
                key={index}
              >
                <Link key={index} href={`/paket/${item.id}`}>
                  <Card shadow="lg" className="h-full w-full bg-black/40">
                    <CardHeader className="top-1 z-10 flex-col !items-start px-10 text-2xl tracking-wider text-slate-200">
                      {item.name}
                    </CardHeader>
                    <ul className="mb-2 grid grid-cols-2 gap-3 px-2 text-xl text-white/90 lg:px-10">
                      <li className="col-span-1 flex gap-2">
                        <IconCarFilled />
                        {item.segment}
                      </li>
                      <li className="col-span-1 flex gap-2">
                        <IconEngine />
                        {item.engine_capacity} Motor
                      </li>
                      <li className="col-span-1 flex gap-2">
                        <IconHorse />
                        {item.horsepower}
                      </li>
                      <li className="col-span-1 flex gap-2">
                        <IconSettings />
                        {item.torque}
                      </li>
                      <li className="col-span-1 flex gap-2">
                        <IconBrandSpeedtest />
                        {item.topspeed}
                      </li>
                      <li className="col-span-1 flex gap-2">
                        <IconGasStation />
                        {item.fueltype}
                      </li>
                      <li className="col-span-2 flex gap-2">
                        <IconBattery3 />
                        Yakıt Tüketimi: {item.fuel}/100km
                      </li>
                      <li className="col-span-2 flex gap-3">
                        <IconChartCovariate />
                        0-100 Hızlanma: {item.accerelation}
                      </li>
                      <li className="col-span-2 flex gap-3">
                        <IconManualGearbox />
                        {item.transmission}
                      </li>
                    </ul>
                  </Card>
                </Link>
              </div>
            ))}
          </div>
        </div>
        <br></br>
        <br />
      </div>
      <br />
    </>
  );
}
